...
const defaultOgImage = config.site?.defaultImages?.openGraph;
const defaultTwitterImage = config.site?.defaultImages?.twitter;
...
export type SeoMaxType = z.infer<ReturnType<typeof seoMaxSchema>>;

export const seoMaxSchema = ({ image }: SchemaContext) =>
  z.object({
    title: z
      .string()
    ...
    openGraph: z
      .object({
        ...
        image: z
          .preprocess((val) => (val ? val : defaultOgImage?.imagePath), image())
...
export const twitterSchema = ({ image }: SchemaContext) =>
  z
    .object({
      ...
      image: z
        .preprocess(
          (val) => (val ? val : defaultTwitterImage?.imagePath),
          image()
        )
        .optional(),
      imageAlt: z
