...
const main = async () => {
	...
	await run("git push");
	await run("pnpm --filter astro-seo-max build");
	await run("pnpm changeset publish");
	await run("git push --follow-tags");
	const tag = (await run("git describe --abbrev=0")).replace("\n", "");
	await run(
		`gh release create ${tag} --title ${tag} --notes "Please refer to [CHANGELOG.md](https://github.com/hfournier/blob/main/package/CHANGELOG.md) for details."`,
	);
};
